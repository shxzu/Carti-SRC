...
import dev.tenacity.utils.misc.RaycastUtils;
...
public class Scaffold extends Module {
    ...
    private final ModeSetting rotationMode = new ModeSetting("Rotation Mode", "Universal", "Universal", "NCP", "Normal Skidded", "Back", "45", "Enum", "Down", "0");
    ...
    public static ModeSetting sprintMode = new ModeSetting("Sprint Mode", "Vanilla", "Vanilla", "Watchdog NoGround", "Watchdog Jump", "Watchdog Normal", "Cancel");
    ...
    private static final NumberSetting universalRayCast = new NumberSetting("Universal Raycast", 5, 10, 1, 1);
    ...
    public Scaffold() {
        ...
        this.addSettings(countMode, rotations, rotationMode, placeType, universalRayCast, keepYMode, sprintMode, towerMode, swingMode, delay, timer,
    ...
    public void onMotionEvent(MotionEvent e) {
        ...
        if (e.isPre()) {
            ...
            if (sprint.isEnabled() && sprintMode.is("Watchdog Jump") && MovementUtils.isMoving() && mc.thePlayer.isOnGround()) {
            	mc.thePlayer.jump();
            	mc.thePlayer.motionX *= 0.94;
            	mc.thePlayer.motionZ *= 0.94;
            }
        
            if (sprintMode.is("Watchdog Normal") && MovementUtils.isMoving() && mc.thePlayer.isOnGround()) {
            	mc.thePlayer.motionX *= 0.94;
            	mc.thePlayer.motionZ *= 0.94;
            }
            
            if (sprint.isEnabled() && sprintMode.is("Watchdog NoGround") && MovementUtils.isMoving() && !isDownwards()  && !mc.gameSettings.keyBindJump.isKeyDown() && mc.thePlayer.isSprinting()){
            	mc.thePlayer.motionY = 0.0001;
            	mc.thePlayer.isOnGround();
            	if (mc.gameSettings.keyBindJump.isKeyDown()) {
            ...
            if (rotations.isEnabled()) {
                ...
                switch (rotationMode.getMode()) {
                    case "Universal":
                    	 for (float yaw = MovementUtils.getMoveYaww() - 180; yaw < MovementUtils.getMoveYaww() + 180; yaw += universalRayCast.getValue()) {
                             for (float pitch = 90; pitch > -90; pitch -= universalRayCast.getValue()) {
                                 final MovingObjectPosition cursor = RaycastUtils.getMouseOver(new Vector2f(yaw, pitch), searchRange.getValue());
                    ...
                    case "Normal Skidded":
                    	 var yaww = MovementUtils.getRawDirection() - 180f;
                         var pitch = 90f;
                         RotationUtils.setVisualRotations(rotations);;
                                var serverRotation = RotationUtils.serverRotation;
                                 e.setRotations(yaww, pitch);
                                 RandomUtils.nextFloat(80, 120);
                    	break;
            ...
            if (sneak.isEnabled()) 
            	   if (!MovementUtils.isMoving() && mc.theWorld.getBlockState(new BlockPos(mc.thePlayer).down()).getBlock() != Blocks.air ) {
            ...
            if (mc.thePlayer.onGround) {
                keepYCoord = Math.floor(mc.thePlayer.posY - 1);
            }
            }
    ...
    private BlockCache blockCachee, lastBlockCachee;
